    ...
    if config_snapshot_path is not None:
        _save_config_snapshot(
            ...
            payload={
                ...
                "layer_diff_threshold": layer_diff_threshold,
                "async_exchange": async_exchange,
                "beta_schedule": cfg.p2p.beta_schedule,
            },
        )
        ...
        for epoch in range(cfg.max_epochs):
            ...
            if epoch_writer:
                _write_epoch_row(
                    ...
                    file_handle=epoch_file,
                    epoch=epoch + 1,
                    epoch_reward=epoch_reward,
                    epoch_successes=epoch_successes,
                    epoch_collisions=epoch_collisions,
                    epoch_exchanges=epoch_exchanges,
...
def _open_epoch_csv_writer(path: str | None) -> tuple[object | None, csv.DictWriter | None]:
    if path is None:
        return None, None
    out = Path(path)
    out.parent.mkdir(parents=True, exist_ok=True)
...
def _write_epoch_row(
    *,
    epoch_writer: csv.DictWriter | None,
    file_handle: object | None,
    epoch: int,
    epoch_reward: float,
    epoch_successes: int,
